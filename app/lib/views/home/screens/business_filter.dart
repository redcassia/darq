...
class BusinessFilter extends StatefulWidget {
  ...
  String jsonFile;
  BusinessFilter({this.jsonFile});
  @override
  _BusinessFilterState createState() => _BusinessFilterState();
}
  ...
  loadLayoutAndData() {
    rootBundle.loadString(PathFiles.ProfilePath + widget.jsonFile).then((js) {
      ...
      Backend.getClient().then((client) => client
          .query(QueryOptions(
          documentNode: gql(
              _layout["list"]["filter_values_query"]
           )))
          .then((result) {
              ...
              onButtonPressed: () {
                for (int i = 0; i < list.length; i++)
                  if (list[i].isSelected)
                    selectedData.add(list[i].data["name"]);
                Navigator.pushReplacement(
                    context,
                    MaterialPageRoute(
                        builder: (context) =>
                            ListingPage(filteredData: selectedData,jsonFile: widget.jsonFile)));
              }),
